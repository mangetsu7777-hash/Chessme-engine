...
const HASH_MB           = parseInt(process.env.HASH)         || 48;   // 2 engines × 48MB = 96MB hash, comfortably within Railway Hobby
...
class Engine {
  ...
  _onLine(line) {
    ...
    if (line.startsWith('bestmove')) {
      ...
      this.busy    = false;
      resolve({ cp, mate });
      pool.drain(); // let pool pick next job
    }
  }
  ...
  eval(fen, depth) {
    return new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        ...
        pool.drain();
      }, 2000 + 3000); // movetime 2000ms + 3s buffer

      this.busy    = true;
      this.current = { fen, bestCp: null, bestMate: null,
        resolve: r => { clearTimeout(timer); resolve(r); } };
      // No ucinewgame — keep hash table warm across positions (faster)
      this._send(`position fen ${fen}`);
      this._send(`go depth ${depth} movetime 2000`); // 2s cap — good depth on Railway CPU
    });
  }
}
...
const pool = {
  ...
  // Pending: [{ fen, depth, key, resolve, reject, pendingResolvers }]
  pending: [],

  // Dedup map: key → { promise, resolvers[] } — same FEN only evaluated once
  inflight: new Map(),
  ...
  eval(fen, depth) {
    ...
    // Already being evaluated? Attach to same promise
...
// ── Rate limiter (per IP, sliding window 60s) ─────────────────────────────────
...
// ── Keepalive — ping self every 2 min ────────────────────────────────────────
  ...
  if (req.method === 'POST' && req.url === '/eval') {
    ...
    req.on('end', async () => {
      try {
        ...
        // Queue depth check — shed load gracefully
        ...
        const results = await Promise.all(fens.map(async fen => {
          ...
          // 2. Supabase eval_cache (shared across all restarts + users)
          ...
          try {
            ...
            // Write to Supabase async (don't block response)
